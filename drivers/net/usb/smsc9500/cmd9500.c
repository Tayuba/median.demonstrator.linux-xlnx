...
void SetDebugMode(PCOMMAND_DATA commandData, unsigned long debug_mode);
void SetLinkMode(PCOMMAND_DATA commandData, unsigned long link_mode);
void SetPowerMode(PCOMMAND_DATA commandData, unsigned long power_mode);
...
void MacSetReg( PCOMMAND_DATA commandData, unsigned long address, unsigned long data);
void PhyGetReg(PCOMMAND_DATA commandData, unsigned long phyaddress, unsigned long phyindex);
void PhySetReg( PCOMMAND_DATA commandData, unsigned long phyaddress, unsigned long address, unsigned long data);
...
void DisplayUsage(void) {
	...
	printf("       SET_LINK_STATUS = sets the link status\n");
	printf("             (applicable only when driver MdioDisable is set)\n");
	printf("         -d specifies the link status\n");
	printf("             0 = link down, 1 = link up\n");
	printf("       GET_LINK_STATUS = gets the driver's internal link status value\n");
	printf("             (applicable only when driver MdioDisable is set)\n");
	...
	printf("         -n specifies the Phy address (for internal phy use addess 1)\n");
...
/*
 * GetEepromSize() function moved up from the below of the DumpEEPROM()
 * function to the above, otherwise the compiler complains.
 * Also the first reutn was returning no value.
 * Changed to return -1.
 */
...
void SetLinkStatus(PCOMMAND_DATA commandData, unsigned long link_status)
{
	if (link_status <= 1) {
		if (commandData == NULL)
			return;
		commandData->IfReq.ifr_data = (void *)&(commandData->IoctlData);
		commandData->IoctlData.dwSignature = SMSC9500_APP_SIGNATURE;
		commandData->IoctlData.dwCommand = COMMAND_SET_LINK_STATUS;
		commandData->IoctlData.Data[0] = link_status;
		...
		printf("Invalid Link status, %ld\n", link_status);
	}
}

void GetLinkStatus(PCOMMAND_DATA commandData)
{
	if (commandData == NULL)
		return;
	...
	commandData->IfReq.ifr_data = (void *)&(commandData->IoctlData);
	commandData->IoctlData.dwSignature = SMSC9500_APP_SIGNATURE;
	commandData->IoctlData.dwCommand = COMMAND_GET_LINK_STATUS;

	ioctl(commandData->hSockFD, SMSC9500_IOCTL, &(commandData->IfReq));

	if (commandData->IoctlData.dwSignature == SMSC9500_DRIVER_SIGNATURE) {
		unsigned long link_status = commandData->IoctlData.Data[0];
		printf("link_status == 0x%02lX\n", link_status);
	} else {
...
void PhyGetReg(PCOMMAND_DATA commandData, unsigned long phyAddress, unsigned long phyindex)
{
	...
	commandData->IoctlData.Data[0] = phyindex;
	commandData->IoctlData.Data[1] = phyAddress;

	ioctl(commandData->hSockFD, SMSC9500_IOCTL, &(commandData->IfReq));

	if (commandData->IoctlData.dwSignature == SMSC9500_DRIVER_SIGNATURE) {
		printf("PhyAddress = 0x%08lX, Phy Index = 0x%08lX, Read Value = 0x%08lX\n", 
			phyAddress, commandData->IoctlData.Data[0], commandData->IoctlData.Data[1]);
	} else {
		printf("Failed to Read Phy Register\n");
	}
}

void PhySetReg( PCOMMAND_DATA commandData, unsigned long phyAddress,
        unsigned long address, unsigned long data)
{
	...
	commandData->IoctlData.Data[0] = address;
	commandData->IoctlData.Data[1] = data;
	commandData->IoctlData.Data[2] = phyAddress;
...
void SetEeprom( PCOMMAND_DATA commandData, unsigned long address, unsigned long data)
{
	if(commandData==NULL)
		return;
...
void SetEepromBuffer(PCOMMAND_DATA commandData, unsigned long address, unsigned int len, char* buf)
{
	if (commandData == NULL)
		return;
	commandData->IfReq.ifr_data = (void *)&(commandData->IoctlData);
...
void process_requests(PCOMMAND_DATA commandData)
{
	...
	while(ReceiveULong(server_sock, &requestCode)) {
		...
		case COMMAND_SET_FLOW_PARAMS:
			...
			if (!ReceiveULong(server_sock, &data))
				break;
...
int ac; char * av[];
{
	...
	while((oc = getopt(ac, av, "hH:p:e:c:a:d:f:n:")) != -1) {
		switch(oc) {
		case 'h'://help
			goto BAD_USAGE;
		case 'H'://Host address
		...
		case 'n':
			...
			if (!ParseNumber(optarg, &phyaddress)) {
				goto BAD_USAGE;
			}
			if (phyaddress > 31)
				goto BAD_USAGE;
		...
		case 'c':
			...
			} else if (strcmp(optarg,"SET_LINK_STATUS") == 0) {
				commandCode = COMMAND_SET_LINK_STATUS;
			} else if (strcmp(optarg,"GET_LINK_STATUS") == 0) {
				commandCode = COMMAND_GET_LINK_STATUS;
	...
	case COMMAND_SET_LINK_STATUS:
		if (!dSet)
			goto BAD_USAGE;
		SetLinkStatus(&commandData, data);
		break;

	case COMMAND_GET_LINK_STATUS:
		GetLinkStatus(&commandData);
		break;
	...
	case COMMAND_PHY_GET_REG:
		...
		if (!nSet)
			goto BAD_USAGE;
		printf("phyaddress = %x offset=%x\n", (int)phyaddress, (int)address);
		PhyGetReg(&commandData, phyaddress, address);
		break;
	...
	case COMMAND_PHY_SET_REG:
		...
		if (!nSet)
			goto BAD_USAGE;
		printf("phyaddress = %x offset=%x data=%x\n", (int)phyaddress, (int)address, (int)data);
		PhySetReg(&commandData, phyaddress, address, data);
		break;
